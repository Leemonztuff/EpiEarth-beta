...
const ItemStrategies: Record<string, (ctx: ItemEffectContext) => ItemEffectResult> = {
    'heal_hp': ({ item, stats }) => {
        ...
        // Fixed: Use fixedValue instead of amount
        const amount = item.id.includes('potion') ? rollDice(4, 2) + 2 : (item.effect?.fixedValue || 0);
    ...
    'restore_mana': ({ item, stats }) => {
        // Fixed: Use fixedValue instead of amount
        const amount = item.effect?.fixedValue || 1;
