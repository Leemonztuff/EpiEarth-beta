...
const BattleBackground = () => {
    ...
    const backgroundUrl = AssetManager.getSafeSprite('/assets/backgrounds/forest_bg.png');
    const treeUrl = AssetManager.getSafeSprite('/assets/sprites/tree_1.png');
    const treeOffsets = [-40, -30, -20, -10, 0, 10, 20, 30, 40];
    ...
    return (
        <group ref={bgRef}>
            <Image url={backgroundUrl} scale={[100, 50]} position={[0, 10, -30]} />
            
            {/* Billboard Trees */}
            {treeOffsets.map((x, index) => (
                 <Billboard key={x} position={[x + ((index % 3) - 1) * 2, -2, -15 - (index % 4) * 2]}>
                    <Image url={treeUrl} scale={15} />
                </Billboard>
            ))}
        </group>
    );
...
const Character = ({ spriteUrl, position, isPlayer }) => {
    ...
    const safeSpriteUrl = spriteUrl ? AssetManager.getSafeSprite(spriteUrl) : undefined;

    return (
        <group position={position}>
            {safeSpriteUrl && !imgError ? (
                <Billboard>
                    <Image 
                        url={safeSpriteUrl}
                        scale={[3, 3]}
