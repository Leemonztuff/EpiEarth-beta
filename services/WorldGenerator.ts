...
export class WorldGenerator {
    ...
    static getTile(q: number, r: number, dimension: Dimension): HexCell {
        ...
        let baseTerrain = TerrainType.GRASS;
        let overlayTerrain: TerrainType | null = null;
        if (noise < 0.1) baseTerrain = TerrainType.MOUNTAIN;
        else if (noise < 0.2) baseTerrain = TerrainType.WATER;
        else if (noise < 0.3) {
            baseTerrain = TerrainType.GRASS;
            overlayTerrain = TerrainType.FOREST;
        }
        else if (noise < 0.35) baseTerrain = TerrainType.DESERT;
        else if (noise < 0.4) baseTerrain = TerrainType.SWAMP;

        if (dimension === Dimension.UPSIDE_DOWN) {
            if (baseTerrain === TerrainType.GRASS) baseTerrain = TerrainType.WASTELAND;
            if (baseTerrain === TerrainType.WATER) baseTerrain = TerrainType.CHASM;
            if (overlayTerrain === TerrainType.FOREST) {
                overlayTerrain = TerrainType.RUINS;
            }
        }
        ...
        if (!overlayTerrain && baseTerrain === TerrainType.GRASS && !poiType && rng.next() < 0.15) {
            feature = 'tree';
            featureSprite = 'scenery/tree.png';
        }
        if (poiType === 'CITY') {
            overlayTerrain = TerrainType.VILLAGE;
            feature = undefined;
            featureSprite = undefined;
        }
        if (poiType === 'VILLAGE') {
            overlayTerrain = TerrainType.VILLAGE;
            feature = undefined;
        ...
        if (poiType === 'DUNGEON' || poiType === 'TEMPLE') {
            overlayTerrain = TerrainType.RUINS;
            feature = undefined;
            featureSprite = undefined;
        }

        const finalTerrain = overlayTerrain ?? baseTerrain;
        ...
        return {
            q, r, terrain: finalTerrain, baseTerrain, overlayTerrain,
        ...
        for (let dq = -3; dq <= 3; dq++) {
            for (let dr = -3; dr <= 3; dr++) {
                if (Math.abs(dq + dr) <= 3) {
                    ...
                    cells.push({
                        q: dq, r: dr, terrain, baseTerrain: terrain, overlayTerrain: null,
