...
import { HexCell, BattleCell, CharacterClass, TerrainType, MovementType } from '../types';
...
export const findPath = (
    ...
    const endCost = endCell.movementType === movementType ? 1 : (TERRAIN_MOVEMENT_COST[movementType]?.[endCell.terrain] || 1);
    if (endCost >= 99) return null;
    ...
    while (openSet.length > 0) {
        ...
        for (const dir of HEX_DIRECTIONS) {
            ...
            const cost = neighbor.movementType === movementType ? 1 : (TERRAIN_MOVEMENT_COST[movementType]?.[neighbor.terrain] || 1);
            if (cost >= 99) continue;
...
export const findBattlePath = (
    ...
    if (targetCell?.isObstacle && movementType !== MovementType.FLY) return null;
    ...
    while (openSet.length > 0) {
        ...
        for (const dir of GRID_DIRECTIONS) {
            ...
            if (neighbor.isObstacle && movementType !== MovementType.FLY) continue;
            
            if (occupied && occupied.has(nKey) && nKey !== endKey) continue;

            const heightDiff = (neighbor.offsetY + neighbor.height) - (current.cell.offsetY + current.cell.height);
            if (heightDiff > 1.2 && movementType !== MovementType.FLY) continue; 
            ...
            if (existingNode) {
                ...
                openSet.push({ cell: neighbor, g: tentativeG, f: tentativeG + heuristic, parent: current });
            }
        }
    }
    return null;
